...
import os
import random
from utils.util import training_scheduler, sort_training_nodes, setup_seed, get_noisy_multilabel_data, sort_mccl
...
def run_model_DBLP(args,lamb,big_t,i, noise_rate):
    ...
    for train_round in range(1):
        ...
        # training loop
        early_stopping = EarlyStopping(patience=args.patience, verbose=True, save_path='/MDCL/GNN/checkpoint/checkpoint_{}_{}.pt'.format(args.dataset, args.num_layers))
        if not mccl:
            if args.dm >= 1:
                if args.dm == 1: # Topo
                    alpha = [0,1,1,1,0]
        ...
        for epoch in range(args.epoch):
            ...
            if args.dm == 0:
                ...
                train_loss = loss(logp[train_idx], labels[train_idx]).mean(dim=1)
                if epoch == 0:
                ...
                loss = nn.BCELoss()
                train_loss = loss(logp[train_mask], labels[train_mask])
            # autograd
            optimizer.zero_grad()
            train_loss.backward()
            optimizer.step()
            ...
            # early stopping
            # wandb.log({f"epoch":epoch, f"test_macro":test['macro-f1'], f"test_micro":test['macro-f1'],f"loss": train_loss})
        ...
        net.load_state_dict(torch.load('/MDCL/GNN/checkpoint/checkpoint_{}_{}.pt'.format(args.dataset, args.num_layers)))
        net.eval()
        test_logits = []
        with torch.no_grad():
            logits = net(features_list)
...
if __name__ == '__main__':
    ...
    ap.add_argument('--num-heads', type=int, default=4, help='Number of the attention heads. Default is 4.')
    ...
    ap.add_argument('--model-type', type=str,default='gat')
