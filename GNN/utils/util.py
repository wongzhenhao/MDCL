...
def feature_difficulty_measurer(data, label, embedding):
    ...
    diff /= 5
    return torch.tensor(diff).to(device)

def feature_difficulty_measurer_2(data, label, embedding):
    normalized_embedding = F.softmax(embedding)
    classes = label.unique()
    class_features = {}
    global_difficulty = []
...
def difficulty_measurer(data, label, embedding): 
    ...
    averaged_loss = torch.load('/MDCL/GNN/averaged_loss.pt')
    ...
    torch.save(diff,'/MDCL/GNN/diff_gcn.pt')
    return diff
...
def weighting(data, label, embedding, alpha, loss_alpha):
    correlation = []
    if os.path.exists('/MDCL/GNN/diff_gcn.pt'):
        diff = torch.load('/MDCL/GNN/diff_gcn.pt')
    else:
...
def sort_training_nodes(data, label, embedding, alpha, loss_alpha):
...
def get_noisy_multilabel_data(data, percent, attack='uniform'):
    data.train_id = torch.tensor(data.train_mask).nonzero().squeeze(dim=1)
    noisy_data = copy.deepcopy((data))
    ...
    val_ids = torch.nonzero(torch.tensor(noisy_data.val_mask)).squeeze(dim=1).cpu() 
    np.random.shuffle(val_ids.numpy())
    val_wrong_ids = val_ids[:int(percent * val_ids.shape[0])].to(device)
